...
class KernelPrior(nn.Module):
    def __init__(
        ...
        for _ in range(n_blocks):
            ...
            mask = (
                1 - mask
            )  # like permutation, though a waste of parameters in the first layer
    ...
    def log_prob(self, x, y=None):
        ...
        return (
            self.base_dist.log_prob(u).sum(1) + sum_log_abs_det_jacobians,
            u,
        )  # should all be summation
...
class LinearMaskedCoupling(nn.Module):
    ...
    def inverse(self, u, y=None):
        ...
        # run through model
        log_s = self.s_net(
            mu if y is None else torch.cat([y, mu], dim=1)
        )  # log of scale, log(s)
