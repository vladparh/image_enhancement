...
from src.models.naf_net.model.local_arch import Local_Base
from src.models.naf_net.model.NAFNet_arch import LayerNorm2d
from src.models.ufpdeblur.Flow_arch import KernelPrior
from src.models.ufpdeblur.my_module import code_extra_mean_var
...
class kernel_attention(nn.Module):
    def __init__(self, kernel_size, in_ch, out_ch):
        ...
        self.conv_1 = nn.Sequential(
            nn.Conv2d(in_ch, out_ch, kernel_size=3, stride=1, padding=1), nn.GELU()
        )
        self.conv_kernel = nn.Sequential(
            nn.Conv2d(
                kernel_size * kernel_size, out_ch, kernel_size=3, stride=1, padding=1
            ),
            nn.GELU(),
...
class NAFBlock_kernel(nn.Module):
    def __init__(self, c, DW_Expand=2, FFN_Expand=2, drop_out_rate=0.0, kernel_size=21):
        super().__init__()
        dw_channel = c * DW_Expand
        self.conv1 = nn.Conv2d(
            in_channels=c,
...
class UFPNet_code_uncertainty(nn.Module):
    def __init__(
        ...
        self.flow = KernelPrior(
            n_blocks=5, input_size=19**2, hidden_size=25, n_hidden=1, kernel_size=19
        )
        ...
        for num in enc_blk_nums:
            if num == 1:
                self.encoders.append(
                    nn.Sequential(
                        *[
                            NAFBlock_kernel(chan, kernel_size=kernel_size)
                            for _ in range(num)
                        ]
                    )
                )
            ...
            self.kernel_down.append(
                nn.Conv2d(kernel_size * kernel_size, kernel_size * kernel_size, 2, 2)
            )
            chan = chan * 2
    ...
    def forward(self, inp):
        ...
        with torch.no_grad():
            ...
            kernel_code = (
                kernel_code - torch.mean(kernel_code, dim=[2, 3], keepdim=True)
            ) / torch.std(kernel_code, dim=[2, 3], keepdim=True)
            # code uncertainty
            sigma = kernel_var
            kernel_code_uncertain = (
                kernel_code * torch.sqrt(1 - torch.square(sigma))
                + torch.randn_like(kernel_code) * sigma
            )

            kernel = generate_k(
                self.flow,
                kernel_code_uncertain.reshape(
                    kernel_code.shape[0] * kernel_code.shape[1], -1
                ),
            )
            ...
            kernel = kernel.permute(0, 2, 3, 1).reshape(
                B, self.kernel_size * self.kernel_size, inp.shape[2], inp.shape[3]
            )
        ...
        for encoder, down, kernel_down in zip(
            self.encoders, self.downs, self.kernel_down
        ):
            if len(encoder) == 1:
