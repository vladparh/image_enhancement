...
class kernel_extra_Decoding_Block(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        self.Conv_head = nn.Conv2d(
            out_ch * 2, out_ch, kernel_size=3, stride=1, padding=1
        )
        self.ResBlock1 = ResBlock(out_ch)
        self.ResBlock2 = ResBlock(out_ch)
        self.act = nn.ReLU()
    ...
    def forward(self, input, skip):
        output = self.Conv_t(
            input,
            output_size=[skip.shape[0], skip.shape[1], skip.shape[2], skip.shape[3]],
        )
...
class kernel_extra_conv_tail(nn.Module):
    ...
    def forward(self, input):
        ...
        kernel_mean = kernel_mean.reshape(
            kernel_mean.shape[0],
            self.kernel_size,
            self.kernel_size,
            kernel_mean.shape[2] * kernel_mean.shape[3],
        ).permute(0, 3, 1, 2)
...
class kernel_extra_conv_tail_mean_var(nn.Module):
    ...
    def forward(self, input):
        ...
        kernel_var = kernel_var.reshape(
            kernel_var.shape[0],
            self.kernel_size,
            self.kernel_size,
            kernel_var.shape[2] * kernel_var.shape[3],
        ).permute(0, 3, 1, 2)
...
class code_extra_mean_var(nn.Module):
    def __init__(self, kernel_size):
        ...
        self.Decoding_Block1 = kernel_extra_Decoding_Block(256, 128)
        self.Decoding_Block2 = kernel_extra_Decoding_Block(128, 64)
        self.Conv_tail = kernel_extra_conv_tail_mean_var(
            64, self.kernel_size * self.kernel_size
        )
