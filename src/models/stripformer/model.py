...
class Inter_SA(nn.Module):
    def __init__(self, dim, head_num):
        ...
        self.conv_h = nn.Conv2d(
            self.hidden_size // 2, 3 * (self.hidden_size // 2), kernel_size=1, padding=0
        )  # qkv_h
    ...
    def forward(self, x):
        ...
        if H == W:
            ...
            attention_output_v = attention_output[1]
            attention_output_h = (
                attention_output_h.view(B, H, C // 2, W)
                .permute(0, 2, 1, 3)
                .contiguous()
            )
            attention_output_v = (
                attention_output_v.view(B, W, C // 2, H)
                .permute(0, 2, 3, 1)
                .contiguous()
            )
            attn_out = self.fuse_out(
